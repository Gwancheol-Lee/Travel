...
<body>
	...
	<div class="container main" align="center">
		<h1 class="my">마이페이지</h1>
		<div align="right">
			<p class="my_p">
				<b style="color: purple;">${member.member_name}</b>님의 마이페이지입니다
				&nbsp; 회원 가입일: ${member.member_date}
			</p>
			<p class="my_p">
				현재 주문 물품개수:<b style="color: green;"> &nbsp; ${map.count} &nbsp;</b>개
			</p>
		</div>
	</div>
	<div class="container main2">
		...
		<ul class="nav nav-tabs">
			...
			<!-- 내가 구매한 글 목록 -->
			<div id="mypay" class="container tab-pane active">
						...
						<c:choose>
							...
							<c:when test="${not empty row}">
								<tr bgcolor="#fff" height="50">
									...
									<td>${row.delivery}</td>
									<%-- <td><input type="checkbox" name="d_check" id="d_check" data-paymentNum="${row.payment_id}"></td> --%>
								</tr>
							</c:when>
							<%-- 검색결과가 없을 떄 --%>
							<c:when test="${map.count == 0}">
								<tr style="text-align: center;">
									<td colspan='5'><b>구매내역이 없습니다.</b></td>
								</tr>
							</c:when>
						</c:choose>
					</c:forEach>
				...
				<%-- <hr class="my-4">
				<div class="btn_box">
					<button class="d_button" data-paymentNum="${row.payment_id}" >삭제하기</button>
				</div> --%>
			</div>
					...
					<table class="table">
						...
						<tr>
							...
							<td><input type="password" id="chk_member_pwd" class="input"
								maxlength="20" /> <label id="checkPasswd"></label></td>
						</tr>
						<tr>
							<th>이름</th>
